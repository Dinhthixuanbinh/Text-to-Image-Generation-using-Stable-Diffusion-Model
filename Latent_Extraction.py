...
with torch.no_grad():
    ...
    if train_config['save_latents']:
        ...
        for idx, im in enumerate(tqdm(data_loader)):
            im = im[0].float().to(device)  # Convert images to float tensors
            encoded_output = vae.encode(im).latents
            fname_latent_map[im_dataset.images[idx]] = encoded_output.to(device)
